...
function wptuts_get_settings() {
	...
	$output['wptuts_page_title'] 		= __( 'Opciones adicionales','wptuts_textdomain'); // the settings page title
	// Fin de línea modificada - sitio web UEC
	
	$output['wptuts_page_sections'] 	= wptuts_options_page_sections(); // the setting section
	$output['wptuts_page_fields'] 		= wptuts_options_page_fields(); // the setting fields
	$output['wptuts_contextual_help'] 	= wptuts_options_page_contextual_help(); // the contextual help
...
function wptuts_add_menu(){
	...
	$wptuts_settings_page = add_theme_page(__('Wptuts Options'), __('Opciones adicionales','wptuts_textdomain'), 'manage_options', WPTUTS_PAGE_BASENAME, 'wptuts_settings_page_fn');
...
function wptuts_form_field_fn($args = array()) {
	...
	switch ( $type ) {
		...
		case 'textarea':
			...
			echo "<textarea class='textarea$field_class' type='text' id='$id' name='" . $wptuts_option_name . "[$id]' rows='10' cols='70'>$options[$id]</textarea>";
			echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : ""; 		
		break;
...
?>
	<div class="wrap">
		...
		<form action="options.php" method="post">
			...
			<p class="submit">
				<input name="Submit" type="submit" class="button-primary" value="<?php esc_attr_e('Guardar cambios','wptuts_textdomain'); ?>" />
			</p>
